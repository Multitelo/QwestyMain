...
const CoinGrid = ({ balance = 360, coins = 6 }) => {
  return (
    <div>
      ...
      <div className="coin_grid grid grid-cols-3 w-fit gap-x-1 gap-y-2">
        {Array.from({ length: coins }).map((coin, index) => (
          <div key={index} className="bg-white w-14 sm:w-16 px-5 py-[1px] rounded-lg flex justify-center items-center">
            {index !== 5 && <img src={qwestCoin} alt="coin" className=""  />}
          </div>
        ))}
      </div>
    </div>
